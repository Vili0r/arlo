...
| **DESIGN-021** | Due Date Lock Guard & Extension Request Service | `lib/actions/capa.ts`<br>`components/capa-edit-form.tsx` | Enforces phase due-date immutability in `updateCapa`, governs `ExtensionRequest` submission and approval/rejection lifecycle, and modifies target phase due dates upon authorized review with 21 CFR Part 11 audit trails. |
| **DESIGN-022** | CAPA Phase Locking & Approver Gatekeeper | `components/capa-edit-form.tsx`<br>`lib/actions/capa.ts`<br>`lib/actions/esignature.ts` | Enforces field & tab lock barriers before and after `INVESTIGATION` phase, phase approval workflows, and blocks lifecycle advancing in UI and backend without formal approver sign-off. |
